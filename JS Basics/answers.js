...
let y = 10;
console.log(y);
// 10 (y is initialized before logging)
// // Q2
let a = 2;
a = 5;
console.log(a);
// 5 (let can be reassigned but not redeclared)
// // Q3
let b = 100;
{
    let b = 50;
...
console.log(b);
//50 100 (b=50 block scope so log 50 outside block can not see it so log 100)
...
// // Q5
let x = 42;
console.log(typeof global.x === 'undefined' ? 'undefined' : global.x === x);
//true (when initialize x without declaring using var, let, const will be global variable)
// // Q6
function test() {
    let z = 1;
    return function () {
...
// function (function declaration is hoisted), but in strict mode will get undefined
...
//Undefined  7  arrow functions don't use the object itself as this but in regular functions use object as this
...
console.log(g());
// 99 g will execute with id in bind
...
const g = f;
f = null; // will get error const cannot be reassigned
console.log(g(4));
// if edit it instead of const => let will log 24 (f named function expression so when edit the reference not affect on recursion)
...
// to log 0 1 2 just replace var with let=> create new variable containing value for each iteration
